...
//				System.out.println("END OF FILE");
			...
			case START:
				if(Character.isWhitespace(ch)){
					...
					token.setTokenvalue(token.getTokenvalue() + ch);
					...
					token.setTokenvalue(ch + "");
					token.setTokenType(getSymbolTokenType(ch));
				}else{
					currentState= State.DONE;
					//END OF FILE
					token.setTokenType(Token.TokenType.END_OF_FILE);
				}
				break;
			case INCOMMENT:
				if(ch == '}'){
			...
			case INID:
				if(Character.isLetter(ch)){
					...
					token.setTokenType(getIdentifierTokenType(token.getTokenvalue()));	//to see if the identifier is one of the reserved words and return the correct TokenType
				}
				break;
			case INNUM:
	...
	private Token.TokenType getIdentifierTokenType(String identifier) {
		...
		return 	identifier.equals("if")? 		Token.TokenType.IF :
				...
				identifier.equals("write") ? 	Token.TokenType.WRITE :	Token.TokenType.IDENTIFIER;
	}
	...
	private Token.TokenType getSymbolTokenType(char symbol) {
		return 	symbol == '+'?	Token.TokenType.PLUS :
				...
				symbol == '/'?	Token.TokenType.DIVISION :
				symbol == '='?	Token.TokenType.EQUAL :
				symbol == '<'?	Token.TokenType.LESSTHAN :
				symbol == '>'?	Token.TokenType.GREATERTHAN :
				symbol == '('?	Token.TokenType.OPENBRACKET :
				symbol == ')'?	Token.TokenType.CLOSEDBRACKET : Token.TokenType.SEMICOLON;
	}
	...
	public static void main(String[] args) throws IOException{
		...
		while(scanner.inputFile.getFilePointer() <= scanner.inputFile.length()){
			...
			if(t.getTokenType() == Token.TokenType.END_OF_FILE)	break;
			if (t.getTokenType() == null || t.getTokenvalue() == ""){
